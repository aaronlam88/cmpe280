...
clockCtr.drawTime = function drawTime(ctx, radius) {
    ...
    hour = (hour * Math.PI / 6) +
        ...
        clockCtr.drawHand(ctx, hour, radius * 0.5, radius * 0.07);
    //minute
    minute = (minute * Math.PI / 30) + (second * Math.PI / (30 * 60));
    clockCtr.drawHand(ctx, minute, radius * 0.8, radius * 0.07);
    // second
    second = (second * Math.PI / 30);
    clockCtr.drawHand(ctx, second, radius * 0.9, radius * 0.02);
};
